      ...
      {hunks.map((hunk, index) => (
        <div key={index} className="border-t border-gray-200 first:border-t-0">
          {hunk.changes && hunk.changes.map((change, changeIndex) => {
            if (viewMode === 'change' ||
               (viewMode === 'previous' && change.type !== 'insert') ||
               (viewMode === 'new' && change.type !== 'delete')) {
              return (
                <DiffLine
                  key={changeIndex}
                  type={change.type}
...
const ViewModeBadge = ({ mode, currentMode, onClick }) => (
  <Badge
              ...
              <article className="w-full lg:w-2/3">
                <div className="max-h-screen overflow-y-auto">
                  {files.map((file, index) => (
                    <DiffFile key={index} {...file} viewMode={viewMode} />
                  ))}
                </div>
              </article>
